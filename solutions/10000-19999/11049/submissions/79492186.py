...
def main():
    ...
    while q:
        ...
        if union_parent(parent,l_idx,r_idx) == True:
            ...
            if l_idx != 0:
                heapq.heappush(q,(matrix[l_idx-1][1]*matrix[l_idx][0]*matrix[l_idx][1],l_idx-1,r_idx))
            if r_idx + 1 < n:
                heapq.heappush(q,(matrix[r_idx][0]*matrix[r_idx][1]*matrix[r_idx+1][1],l_idx,r_idx+1))
    print(answer)
