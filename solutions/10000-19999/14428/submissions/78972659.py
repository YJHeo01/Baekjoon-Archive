...
def init(array,s,node,start,end):
    ...
    if array[s[l_node]] > array[s[r_node]]: s[node] = s[l_node]
    else: s[node] = s[r_node]

def update(array,s,node,target,start,end):
    if start > target or end < target or start == end: return s[node]
    l_node = node*2; r_node = l_node + 1; mid = (start+end) // 2
    l_min = update(array,s,l_node,target,start,mid)
    r_min = update(array,s,r_node,target,mid+1,end)
