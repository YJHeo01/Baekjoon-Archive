...
def get_new_blue_block(blue,x1,x2):
    y = 2
    while True:
        if y > 5 or blue[x1][y] == True or blue[x2][y] == True:
            y -= 1
            break
        y += 1
...
def get_new_green_block(green,y1,y2):
    x = 2
    while True:
        if x > 5 or green[x][y1] == True or green[x][y2] == True:
            x -= 1
            break
        x += 1
...
def get_blue_board_score(blue):
    ...
    for column in range(start,end,-1):
        if check_full_column(blue,column) == True:
            ret_value += 1
            end += 1
...
def get_green_board_score(green):
    ...
    for row in range(start,end,-1):
        if check_full_row(green,row) == True:
            ret_value += 1
            end += 1
            erase_row(green,row)
