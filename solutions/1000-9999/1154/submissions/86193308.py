...
def main():
    ...
    while True:
        ...
        first_A_team, first_B_team = False, False
        if check_possible_team(friend,A_team,first_idx):
            tmp_A_team.append(first_idx)
            first_A_team = True
        elif check_possible_team(friend,B_team,first_idx):
            tmp_B_team.append(first_idx)
            first_B_team = True
        else:
            print(-1)
            exit(0)
        ...
        for i in range(first_idx+1,n+1):
            if friend[first_idx][i] == False: 
                if first_A_team: tmp_B_team.append(i)
                else: tmp_A_team.append(i)
        for i in tmp_A_team:
            finish[i] = True
            for j in tmp_A_team:
                if i == j: continue
