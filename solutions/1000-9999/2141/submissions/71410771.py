...
for i in range(1,n+1):
    town_information.append(list(map(int,input().split()))+[i])

town_information.sort()

town_information = [[0,0,0]] + town_information
...
for i in range(1,n):
    ...
    if min_sum_distance > tmp:
        ...
        answer = town_information[i][2]
    elif min_sum_distance == tmp:
        answer = min(answer,town_information[i][2])
