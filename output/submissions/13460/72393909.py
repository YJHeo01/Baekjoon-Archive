...
def control(graph,Red,Blue,move_cnt):
    ...
    ret_value = control(graph,move_north_Red,move_north_Blue,move_cnt+1)
    if ret_value == move_cnt + 1:
        return ret_value
    
    move_south_Red, move_south_Blue = move_south(graph,Red,Blue)
    ret_value = min(control(graph,move_south_Red,move_south_Blue,move_cnt+1),ret_value)
    if ret_value == move_cnt + 1:
        return ret_value
    
    move_right_Red, move_right_Blue = move_right(graph,Red,Blue)
    ret_value = min(ret_value,control(graph,move_right_Red,move_right_Blue,move_cnt+1))
    if ret_value == move_cnt+1:
        return ret_value
    
    move_left_Red, move_left_Blue = move_left(graph,Red,Blue)
    ret_value = min(control(graph,move_left_Red,move_left_Blue,move_cnt+1),ret_value)
    return ret_value
