def main():
    ...
    for x in range(n):
        for y in range(n):
            ...
            make_new_bishop(impossible_new_bishop,(x,y))
            point = get_next_point(x,y)
            answer = max(answer,solution(board,impossible_new_bishop,point,1))
            remove_bishop(impossible_new_bishop,(x,y))
    print(answer)
...
def solution(board,impossible_new_bishop,point,bishop_cnt):
    ...
    if board[x][y] == 1 and impossible_new_bishop[x][y] == 0:
        ...
        ret_value = solution(board,impossible_new_bishop,(nx,ny),bishop_cnt+1)
        remove_bishop(impossible_new_bishop,point)
    else: ret_value = solution(board,impossible_new_bishop,(nx,ny),bishop_cnt)
    return ret_value
