...
def solution(row_block,column_block,water):
    ...
    for i in range(n):
        if column_block[i][0] != -1:
            if water[i][0] <= column_block[i][0]: continue
            water[i][0] = column_block[i][0]
            heapq.heappush(q,(water[i][0],i,0))
        if column_block[i][m] != -1:
            if water[i][m-1] <= column_block[i][m]: continue
    ...
    while q:
        ...
        for dx,dy in [(0,1),(0,-1),(1,0),(-1,0)]:
            ...
            if water[nx][ny] > hole_high and hole_high >= water[vx][vy]:
                water[nx][ny] = hole_high
                heapq.heappush(q,(hole_high,nx,ny))
            if hole_high == -1: continue
            if water[nx][ny] > water[vx][vy] and water[vx][vy] > hole_high:
                water[nx][ny] = water[vx][vy]
                heapq.heappush(q,(water[nx][ny],nx,ny))
    #print_water(water)            
