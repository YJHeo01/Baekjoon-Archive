...
int bfs(vector<vector<bool>>& visited, int x, int y, int ceil, int floor) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0;i < 8;i++) {
            int nx = x + dx[i];
            int ny = y + dy[i];
            if (nx < 0 or ny < 0 or nx >= n or ny >= n or visited[nx][ny]==true) continue;
...
int main() {
    ...
    for (int i = 0;i < n;i++) {
        for (int j = 0;j < n;j++) {
            ...
            floor_max = min(floor_max, high[i][j]);
            ceil_min = max(ceil_min, high[i][j]);
            if (town[i][j] == 'P') {
                start_x = i;
                start_y = j;
            }
