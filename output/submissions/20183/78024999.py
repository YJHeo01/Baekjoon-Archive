...
def main():
    ...
    max_sum_cost = [[0,INF] for _ in range(n+1)]
    answer = get_answer(graph,max_max_cost,max_sum_cost)
    if answer >= INF:
        answer = -1
    print(answer)
...
def get_answer(graph,max_max_cost,max_sum_cost):
    q = []; heapq.heappush(q,(0,0,a))
    max_max_cost[a][1] = 0; max_sum_cost[a][1] = 0
    ret_value = INF
    while q:
        ...
        for nx, cost in graph[vx]:
            ...
            if next_max_cost >= max_max_cost[nx][0]:
                ...
                if max_max_cost[nx][1] >= next_sum_cost: max_max_cost[nx][1] = next_sum_cost; max_max_cost[nx][0] = next_max_cost
            if next_sum_cost >= max_sum_cost[nx][0]:
                if next_max_cost >= max_sum_cost[nx][1]:continue
                max_sum_cost[nx][0] = next_sum_cost; max_sum_cost[nx][1] = next_max_cost
            else:
                if max_sum_cost[nx][1] >= next_max_cost: max_sum_cost[nx][0] = next_sum_cost; max_sum_cost[nx][1] = next_max_cost
