...
def escape_route(graph,visited,start,k):
    queue = deque([start])
    dx = [0,1,0,-1]
    dy = [1,0,-1,0]
    ...
    while queue:
        ...
        for i in range(4):
            nx = vx + dx[i]
            ny = vy + dy[i]
            if nx < min_x_list[k] or ny < min_y_list[k] or nx > max_x_list[k] or ny > max_y_list[k]:
                return True
...
for i in range(island_cnt):
    for j in range(island_cnt):
        if max_x_list[i] > max_x_list[j] and min_x_list[i] < min_x_list[j] and min_y_list[i] < min_y_list[j] and max_y_list[i] > max_y_list[j]:
            ...
            if escape_route(sea,visited,island_point[j],i) == False:
                graph[j].append(i)
                zero_high[i] = False
