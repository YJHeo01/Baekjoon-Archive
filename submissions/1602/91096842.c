...
def main():
    ...
    for _ in range(n+1): floyd(time,delay_time,monkey)
    print_answer(time)
    ...
    for k in range(1,n+1):
        for i in range(1,n+1):
            for j in range(1,n+1):
                ...
                if time[i][j] > path_i_to_k + path_k_to_j + max(delay_time[i][k],delay_time[k][j]):
                    delay_time[i][j] = max(delay_time[i][k],delay_time[k][j])
