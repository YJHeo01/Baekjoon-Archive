...
def solution(sticker,area,use_sticker,point,cnt):
    ...
    ret_value = solution(sticker,area,use_sticker,(vx,vy+1),cnt)
    if cnt == 0: ret_value = 0
    if area[vx][vy] == True:
        return ret_value
    for i in range(n):
        ...
        for j in range(2):
            ...
            if cnt == 1:
                if possible_new_sticker(area,vx,vy,nx,ny) == False: continue
...
def possible_new_sticker(area,vx,vy,nx,ny):
    if vx != 0:
        x = vx - 1
        for y in range(vy,ny):
            if area[x][y] == True:
                return True
    if vy != 0:
        y = vy - 1
        for x in range(vx,nx):
            if area[x][y] == True:
                return True
    return False
