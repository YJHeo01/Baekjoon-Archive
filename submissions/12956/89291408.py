...
def dijkstra(graph,distance,id):
    q = []
    for i in range(n):
        distance[i][i] = 0
        heapq.heappush(q,(0,i,i))
    while q:
        dist, vx, start = heapq.heappop(q)
        if dist > distance[start][vx]: continue
        for nx, dd in graph[vx]:
            if road_id[vx][nx] == id: continue
            nd = dist + dd
            if distance[start][nx] > nd:
                distance[start][nx] = nd
                heapq.heappush(q,(nd,nx,start))
...
for k in range(m):
    ...
    for i in range(n):
        for j in range(i):
            if shortest_path[i][j] != distance[i][j]: answer += 1
    print(answer,end=" ")