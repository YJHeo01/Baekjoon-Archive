...
parent_info = list(range(n))

for i in range(1,n):
    tree[parent[i]-1].append(i)
    if parent_info[parent[i]-1] != 0:
        parent_info[parent_info[i]] = parent_info[parent[i]-1]
...
for _ in range(m):
    ...
    if command == 1:
        i,w = map(int,tmp)
        update(seg_tree,lazy,1,ett[parent_info[i-1]][0],ett[parent_info[i-1]][1],0,n-1,w)
        ...
        if i == 1:
            print(sum_value)
            continue
        print(qeury(seg_tree,lazy,1,ett[i-1][0],0,n-1)+a[i-1])