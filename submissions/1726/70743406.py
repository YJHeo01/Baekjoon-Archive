...
def change_direction(visited,point,change_value):
    if visited[(point[2]+change_value)%4][point[0]][point[1]] > visited[point[2]][point[0]][point[1]] + abs(change_value):
        visited[(point[2]+change_value)%4][point[0]][point[1]] = visited[point[2]][point[0]][point[1]] + abs(change_value)
...
def solution(graph,visited,start):
    ...
    while queue:
        ...
        for i in range(-1,3):
            if i == 0:
                continue
            change_direction(visited,point,i)
            if visited[(point[2]+i)%4][point[0]][point[1]] == visited[point[2]][point[0]][point[1]] + abs(i):
