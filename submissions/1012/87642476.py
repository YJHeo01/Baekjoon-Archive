...
# (dx,dy) -> (x,y)의 좌표를 조정
def bfs(graph,y,x): ##y와 i는 세로축을, x와 j는 가로축을 나타냄
    q = deque([(y,x)]) ## 덱 사용, x,y 와 y,x 순서 통일하셔야 합니다. # q에 입력받은 좌표를 추가
...
for _ in range(t):
    ...
    for _ in range(k):
        ...
        graph[y][x] = 1 # 입력받은 좌표에 배추를 심음

    for i in range(1,n+1): # i는 행의 개수, j는 열의 개수
        for j in range(1,m+1): # 모든 좌표를 둘러봄
            if graph[i][j] == 1: # (i,j)좌표에 배추가 있으면 bfs함수 호출
                bfs(graph, i,j) #y와 i는 세로축을, x와 j는 가로축을 나타냄
