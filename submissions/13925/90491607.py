...
def main():
    ...
    for i in range(m):
        ...
        if t == 4:
            x,y = tmp
            print(query(tree,lazy,1,x-1,y-1,0,n-1))
        else:
            x,y,v = tmp
            command = (i,t,v)
...
def update_lazy(tree,lazy,node,start,end):
    ...
    for order,c,v in lazy[node]:
        ...
        if c == 3:
            ...
            new_lazy.append((order,c,v))
    if start != end:
        lazy[node*2] += new_lazy
        lazy[node*2+1] += new_lazy
    lazy[node] = []
