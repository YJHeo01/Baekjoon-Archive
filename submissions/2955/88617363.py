def main():
    ...
    while True:
        state = min(cross(sudoku,row,column,box),hatching(sudoku,box_pos,box,row,column))
        if state <= 0: break
...
def cross(sudoku,row,column,box):
    for x in range(9):
        for y in range(9):
        ...
        for value in range(1,10):
            ...
            for x in range(start_x,end_x):
                for y in range(start_y,end_y):
                    ...
                    if target_x == -1: target_x, target_y = x,y
                    else: target_x, target_y = -2,-2
